...
void GemmSIMD::packing_A_8x4_neon(const float *A, size_t M, size_t K,
                                  size_t LDA, float *packed_A) {
    float *dst = packed_A;
...
void GemmSIMD::packing_B_8x4_neon(const float *B, size_t K, size_t N,
                                  size_t LDB, float *packed_B) {
...
void GemmSIMD::macro_kernel_4x4_sgemm_neon(size_t M, size_t N, size_t K,
                                           float alpha, const float *A, int LDA,
                                           const float *B, int LDB, float,
                                           float *C, int LDC) {
...
void GemmSIMD::macro_kernel_8x4_sgemm_neon(size_t M, size_t N, size_t K,
                                           float alpha, const float *A, int,
                                           const float *B, int, float, float *C,
                                           int LDC) {
    ...
    for (size_t i = 0; i < M; i += 8) {
        for (size_t j = 0; j < N; j += 4) {
            ...
            for (size_t k = 0; k < K;) {
                KERNEL_8x4_SGEMM_NEON KERNEL_8x4_SGEMM_NEON
                    KERNEL_8x4_SGEMM_NEON KERNEL_8x4_SGEMM_NEON
            }
            ...
            vst1q_f32(&C(i + 4, j + 1),
                      vaddq_f32(c11, vld1q_f32(&C(i + 4, j + 1))));
            vst1q_f32(&C(i, j + 2), vaddq_f32(c20, vld1q_f32(&C(i, j + 2))));
            vst1q_f32(&C(i + 4, j + 2),
                      vaddq_f32(c21, vld1q_f32(&C(i + 4, j + 2))));
            vst1q_f32(&C(i, j + 3), vaddq_f32(c30, vld1q_f32(&C(i, j + 3))));
            vst1q_f32(&C(i + 4, j + 3),
                      vaddq_f32(c31, vld1q_f32(&C(i + 4, j + 3))));
        }
    }
}
