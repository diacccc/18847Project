...
# Run the benchmark
run-single: $(TARGET)
	OMP_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1 OPENBLAS_NUM_THREADS=1 ./$(TARGET) --output results_single.csv

# Run the benchmark
run: $(TARGET)
	OMP_NUM_THREADS=8 VECLIB_MAXIMUM_THREADS=8 OPENBLAS_NUM_THREADS=8 $(OMP_ENV) ./$(TARGET) --output results.csv
