...
extern "C"
{
    ...
    void metal_gemm_execute(void *gemm, float alpha, const float *a, size_t a_rows, size_t a_cols, size_t a_ld,
                            const float *b, size_t b_rows, size_t b_cols, size_t b_ld, float beta, float *c,
                            size_t c_rows, size_t c_cols, size_t c_ld);
...
void GemmMetal::execute(float alpha, const Matrix<float> &A, const Matrix<float> &B, float beta, Matrix<float> &C)
{
    ...
    try
    {
        metal_gemm_execute(metal_gemm_impl, alpha, A.data(), A.rows(), A.cols(), A.ld(), B.data(), B.rows(), B.cols(),
                           B.ld(), beta, C.data(), C.rows(), C.cols(), C.ld());
    }
    catch (const std::exception &e)
    {
