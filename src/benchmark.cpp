...
void GemmBenchmark::addImplementation(
    std::unique_ptr<GemmImplementation> impl) {
    if (impl) {
...
void GemmBenchmark::setMatrixSizes(
...
std::vector<BenchmarkResult> GemmBenchmark::runAll() {
    ...
    if (validate_results_) {
        ...
        if (!ref_impl) {
            std::cout << "Warning: Reference implementation '"
                      << reference_impl_
                      << "' not found. Result validation will be skipped."
                      << std::endl;
    ...
    for (const auto &size : matrix_sizes_) {
        ...
        if (validate_results_ && ref_impl) {
            ...
            std::cout << "Computing reference result using "
                      << ref_impl->getName() << "..." << std::endl;
        ...
        for (const auto &impl : implementations_) {
            ...
            BenchmarkResult result =
                benchmarkImplementation(impl.get(), A, B, C, ref_result.get());
            ...
            std::cout << "Done! " << result.flops / 1e9 << " GFLOPS, "
...
BenchmarkResult GemmBenchmark::benchmarkImplementation(
    GemmImplementation *impl, Matrix<float> A, Matrix<float> B, Matrix<float> C,
    Matrix<float> *reference_result) {
    ...
    // Calculate FLOPS
    double flops = calculateGemmFlops(M, N, K) /
                   (avg_time / 1000.0); // Convert ms to seconds
    ...
    if (reference_result) {
        ...
        if (!result.validated) {
            std::cout << "Warning: Result validation failed for "
                      << impl->getName() << "!" << std::endl;
        }
    }
...
bool GemmBenchmark::saveResults(const std::string &filename,
...
void registerImplementation(const std::string &name,
                            std::function<GemmImplementation *()> factory) {
...
GemmImplementation *createImplementation(const std::string &name) {
    ...
    std::cerr << "Warning: Implementation '" << name << "' not found"
              << std::endl;
    return nullptr;
}
...
void registerImplementations() {
    ...
    registerImplementation(
        "BLAS", []() -> GemmImplementation * { return new GemmBLAS(); });
    registerImplementation(
        "cpu_naive", []() -> GemmImplementation * { return new GemmNaive(); });
    registerImplementation(
        "cpu_simd", []() -> GemmImplementation * { return new GemmSIMD(); });
    ...
    std::cout << "Registered " << implementation_factories.size()
              << " GEMM implementations" << std::endl;
}
